package com.beyonditsm.financial.view.slidebottompanel;
...
/**
 * Created by xuleyuan on 16/6/29.
 */
public class SlideBottomPanel extends FrameLayout {
    ...
    private int mChildCount; //子控件个数
    private float mDensity;
    private boolean isAnimating = false;  //动画是否在运行
    private boolean isPanelShowing = false;  //弹出的Menu是否正在显示
    ...
    public SlideBottomPanel(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mContext = context;
        mDensity = getResources().getDisplayMetrics().density; //得到屏幕密度
