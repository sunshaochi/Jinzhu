...
public class ToggleButton extends View {
	...
	/** 开启颜色 */
	private int onColor = Color.parseColor("#ed6900");
	/** 关闭颜色 */
	private int offBorderColor = Color.parseColor("#e7e7e7");
	/** 灰色带颜色 */
	private int offColor = Color.parseColor("#f5f5f5");
	/** 手柄颜色 */
	...
	private void setup(AttributeSet attrs) {
		...
		borderWidth = typedArray.getDimensionPixelSize(R.styleable.ToggleButton_toggleBorderWidth, borderWidth);
		typedArray.recycle();
	}
	...
	public void toggleIsSwitch(boolean isSwitch) {
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		spotX = spotMinX;
		offLineWidth = height - 2 * borderWidth;
	}
	...
	super.draw(canvas);
		/*
		 * final int height = getHeight(); //绘制背景（边框） paint.setStrokeWidth(height); paint.setColor(borderColor); canvas.drawLine(startX, centerY, endX, centerY, paint); //绘制灰色带 if(offLineWidth > 0){
		 * paint.setStrokeWidth(offLineWidth); paint.setColor(offColor); canvas.drawLine(spotX, centerY, endX, centerY, paint); } //spot的边框 paint.setStrokeWidth(height); paint.setColor(borderColor);
		 * canvas.drawLine(spotX - 1, centerY, spotX + 1.1f, centerY, paint); //spot paint.setStrokeWidth(spotSize); paint.setColor(spotColor); canvas.drawLine(spotX, centerY, spotX + 0.1f, centerY,
		 * paint);
		 */
