...
public class CreditCardItemAdp extends BaseAdapter {
    ...
    private DisplayImageOptions options = new DisplayImageOptions.Builder()
            .showStubImage(R.drawable.bga_refresh_loding) // 设置图片下载期间显示的图片
            .showImageForEmptyUri(R.drawable.bga_refresh_loding) // 设置图片Uri为空或是错误的时候显示的图片
            .showImageOnFail(R.drawable.bga_refresh_loding) // 设置图片加载或解码过程中发生错误显示的图片
...
//        ImageLoader.getInstance().displayImage(IFinancialUrl.SECURITY_IMAGE_URL + datas.get(position).getMobileCreditcardImg(), holder.iv_creditCard, options);
